...
#if ! defined(STREAMTEXTFILE_HPP)
#define STREAMTEXTFILE_HPP
...
struct StreamTextFile : public std::ifstream, public SignatureConstruction<signature_type>
{
        ...
        StreamTextFile(std::string const & filename, unsigned int const rl, unsigned int const rnu)
        : std::ifstream(filename.c_str()), SignatureConstruction<signature_type>(rl,rnu),
          m0(0), m1(0), m2(0), m3(0),
          readFirst(false),
          syms_read(0),
          m_pos(0), peeked(false)
        {
                if ( ! is_open() )
                {
                ...
                // skip first line
                std::getline(*this,fileid);
                
                if ( fileid.size() && fileid[0] == '>' )
                        fileid = fileid.substr(1);
                
                std::deque<std::string> tokens = stringFunctions::tokenize(fileid, std::string(" "));
                
                if ( tokens.size() == 3 )
                        fileid = tokens[2];
                        
                while ( fileid.size() && isspace(fileid[fileid.size()-1]) )
        ...
        {
                int const sym = std::ifstream::get();
                ...
                else
                {
                        ...
                        while ( true )
                        {
                                ...
                                else
                                {
                                        switch ( sym )
                                        {
                                                case 'A': shift(); m3 |= 0; m_pos = syms_read - base_type::l; return true;
                                                case 'C': shift(); m3 |= 1; m_pos = syms_read - base_type::l; return true;
                                                case 'G': shift(); m3 |= 2; m_pos = syms_read - base_type::l; return true;
                                                case 'T': shift(); m3 |= 3; m_pos = syms_read - base_type::l; return true;
                ...
                do
                {
                        ...
                        for ( unsigned int i = 0; read && (i < toread); ++i )
                        {
                                ...
                                else
                                {
                                        switch ( sym )
                                        {
                                                case 'A': shift(); m3 |= 0; break;
                                                case 'C': shift(); m3 |= 1; break;
                                                case 'G': shift(); m3 |= 2; break;
                                                case 'T': shift(); m3 |= 3; break;
        ...
        )
        {
                ...
                if ( i < n && peeked )
                {
                        list_0[ i ].sign = (m0 << base_type::bits_m1) | (m1); list_0[ i ].ptr = i; list_0[ i ].pos = m_pos;
                        list_1[ i ].sign = (m0 << base_type::bits_m2) | (m2); list_1[ i ].ptr = i; list_1[ i ].pos = m_pos;
                        list_2[ i ].sign = (m0 << base_type::bits_m3) | (m3); list_2[ i ].ptr = i; list_2[ i ].pos = m_pos;
