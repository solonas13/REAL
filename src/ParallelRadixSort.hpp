...
struct RadixSort32
{
	...
	static void radixSort(AutoArray<data_type> & AA, u_int64_t n, key_projector const & K, int const inumthreads = -1)
	{
...
#endif
		AutoArray < u_int64_t > Ahist0(3 * numthreads * histsize,false);
...
struct RadixSort64
{
	...
	static void radixSort(AutoArray<data_type> & AA, u_int64_t n, key_projector const & K, int inumthreads = -1)
	{
		data_type * const A = AA.get();
#if defined(_OPENMP)
		unsigned int const maxthreads = omp_get_max_threads( );
		...
		unsigned int const numthreads = 1;
#endif
		AutoArray < u_int64_t > Ahist0(6 * numthreads * histsize,false);
