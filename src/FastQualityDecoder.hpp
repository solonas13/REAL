#if ! defined(FASTQUALITYDECODER_HPP)
#define FASTQUALITYDECODER_HPP
...
#include "FastQualitySubDecoder.hpp"
#include "FastIDDecoder.hpp"
#include "FastFileDecoderBase.hpp"

struct FastQualityDecoder : public FastFileDecoderBase
{
	...
	{
		FastQualityDecoder FD(filename,default_blocksize);
		return FD.numpat;
	}
		
	FastQualityDecoder(std::string const & rfilename, unsigned int const rblocksize)
	: filename(rfilename), numpat(0), blocksize(rblocksize)
	{
		...
		while ( istr )
		{
			...
			if ( istr )
			{
				...
				std::cerr << "Pattern length " << patl << " acgtcodelength=" << acgtcodelength 
					<< " acgtncodelength=" << acgtncodelength << std::endl;
				...
				fdi.patnum = acgtdata/(acgtcodelength+patl);
				...
				fdi.patnnum = acgtndata/(acgtncodelength+patl);
	...
	std::auto_ptr < FastQualitySubDecoder > getDecoder(unsigned int const patl) const
	{
		if ( info.find(patl) != info.end() )
		{
			FastDecodeInfo const & fdi = info.find(patl)->second;			
			return std::auto_ptr < FastQualitySubDecoder > ( new FastQualitySubDecoder (filename, false, fdi.patlen, fdi.patnum, blocksize, fdi.patpos, numbuffers, fdi.idbase) );
		}
		else
		{
			return std::auto_ptr < FastQualitySubDecoder >();
		}
	} 
	std::auto_ptr < FastQualitySubDecoder > getNDecoder(unsigned int const patl) const
	{
		if ( info.find(patl) != info.end() )
		{
			FastDecodeInfo const & fdi = info.find(patl)->second;			
			return std::auto_ptr < FastQualitySubDecoder > ( new FastQualitySubDecoder (filename, true, fdi.patlen, fdi.patnnum, blocksize, fdi.patnpos, numbuffers, fdi.idnbase) );
		}
		else
		{
			return std::auto_ptr < FastQualitySubDecoder >();
